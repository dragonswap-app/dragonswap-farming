...
describe('DragonswapStakerBoosted', () => {
  ...
  before(async () => {
    ...
    boosterAmountWei = parseUnits(
      boosterAmount.toString(),
      boosterTokenDecimals
    );
    rewardPerSecond = 100;
    rewardPerSecondWei = parseUnits(
      rewardPerSecond.toString(),
      rewardTokenDecimals
    );

    const boostedFarmContractFactory = await hre.ethers.getContractFactory(
      'DragonswapStakerFactory'
    );
    const boostedFarmFactory = await boostedFarmContractFactory.deploy(
      owner.address
    );
    await boostedFarmFactory.deployed();

    const boostedFarmImplmentationFactory = await hre.ethers.getContractFactory(
      'DragonswapStakerBoosted'
    );
    const boostedFarmImplementation =
      await boostedFarmImplmentationFactory.deploy();
    await boostedFarmImplementation.deployed();

    await boostedFarmFactory
      .connect(owner)
      .setImplementationBoosted(boostedFarmImplementation.address);
    ...
    boostedFarm = await hre.ethers.getContractAt(
      'DragonswapStakerBoosted',
      boostedFarmTxReceipt.logs[0].address
    );
    ...
    await rewardToken
      .connect(owner)
      .approve(boostedFarm.address, rewardAmountWei);
    await boosterToken
      .connect(owner)
      .approve(boostedFarm.address, boosterAmountWei);
  ...
  describe('When created', function () {
    ...
    it('Should have correct start and end timestamps', async () => {
      ...
      expect(await boostedFarm.endTimestamp()).eq(
        startTimestamp + rewardAmount / rewardPerSecond
      );
    });

    it('Should have correct decimals', async () => {
      expect(await boostedFarm.decimalEqReward()).eq(1);
      expect(await boostedFarm.decimalEqBooster()).eq(
        10 ** (rewardTokenDecimals - boosterTokenDecimals)
      );
    });
  ...
  describe('After 10 seconds of farming', function () {
    ...
    before(async () => {
      await advanceTimeAndBlock(
        startTimestamp - (await currentTimestamp()) + secondsOfFarming
      );
    });
    ...
    it('Has a total of 1000 rewardTokens and 2000 boosterTokens', async () => {
      ...
      expect(totalPendingReward).eq(
        parseUnits(
          (rewardAmount / secondsOfFarming).toString(),
          rewardTokenDecimals
        )
      );
      expect(totalPendingBooster).eq(
        parseUnits(
          (boosterAmount / secondsOfFarming).toString(),
          boosterTokenDecimals
        )
      );
    ...
    describe('With a 3th participant after 30 seconds', function () {
      before(async () => {
        await advanceTimeAndBlock(
          startTimestamp - (await currentTimestamp()) + 28
        );
    ...
    describe('After 50 seconds of farming', function () {
      before(async () => {
        await advanceTimeAndBlock(
          startTimestamp - (await currentTimestamp()) + 50
        );
      });
    ...
    describe('With a participant withdrawing after 70 seconds', function () {
      before(async () => {
        await advanceTimeAndBlock(
          startTimestamp - (await currentTimestamp()) + 69
        );
      ...
      it('Gives alice 3750 rewardToken, 7500 boosterToken and 1500 LP', async () => {
        ...
        expect(balanceAliceBooster).eq(
          parseUnits('7500', boosterTokenDecimals)
        );
      });
    ...
    describe('With a participant partially withdrawing after 80 seconds', function () {
      before(async () => {
        await advanceTimeAndBlock(
          startTimestamp - (await currentTimestamp()) + 79
        );
    ...
    describe('Is safe', function () {
      it("Won't allow alice to withdraw", async () => {
        await expect(
          boostedFarm.connect(alice).withdraw(0, 1500)
        ).to.be.revertedWith('UnauthorizedWithdrawal');
      });
      ...
      it("Won't allow carl to withdraw more than his deposit", async () => {
        ...
        await expect(
          boostedFarm.connect(carl).withdraw(0, 1500)
        ).to.be.revertedWith('UnauthorizedWithdrawal');
      });
    });
    ...
    describe('When it receives more funds(8000 rewardTokens and 16000 boosterTokens)', function () {
      before(async () => {
        await rewardToken.approve(
          boostedFarm.address,
          parseUnits('8000', rewardTokenDecimals)
        );
        await boosterToken.approve(
          boostedFarm.address,
          parseUnits('16000', boosterTokenDecimals)
        );
        await boostedFarm.fund(
          parseUnits('8000', rewardTokenDecimals),
          parseUnits('16000', boosterTokenDecimals)
        );
      });
    ...
    describe('With an added lp token (for 25%) after 100 seconds', function () {
      before(async () => {
        await advanceTimeAndBlock(
          startTimestamp - (await currentTimestamp()) + 99
        );
    ...
    describe('With a first participant for lp2 after 110 seconds', function () {
      before(async () => {
        await advanceTimeAndBlock(
          startTimestamp - (await currentTimestamp()) + 108
        );
    ...
    describe('With second participant for lp2 after 120 seconds', function () {
      before(async () => {
        await advanceTimeAndBlock(
          startTimestamp - (await currentTimestamp()) + 118
        );
    ...
    describe('After 140 seconds of farming', function () {
      before(async () => {
        await advanceTimeAndBlock(
          startTimestamp - (await currentTimestamp()) + 140
        );
      });
    ...
    describe('With a participant partially withdrawing LP2 after 150 seconds', function () {
      before(async () => {
        await advanceTimeAndBlock(
          startTimestamp - (await currentTimestamp()) + 149
        );
    ...
    describe('With a participant doing an emergency withdraw LP2 after 160 seconds', function () {
      before(async () => {
        await advanceTimeAndBlock(
          startTimestamp - (await currentTimestamp()) + 159
        );
    ...
    describe('When closed after 180 seconds', function () {
      before(async () => {
        await advanceTimeAndBlock(
          startTimestamp - (await currentTimestamp()) + 180
        );
      });
    ...
    describe('When closed for 20 seconds (after 200 seconds)', function () {
      ...
      it('Will not accept new funds', async () => {
        await expect(
          boostedFarm.fund(
            parseUnits('1000', rewardTokenDecimals),
            parseUnits('2000', boosterTokenDecimals)
          )
        ).to.be.revertedWith('FarmClosed');
      });
    });
    ...
    describe('With participants withdrawing after closed', function () {
      ...
      it('Gives alice 1250 rewardTokens and 2500 boosterTokens and 1000 LP2', async () => {
        ...
        const balanceAliceBooster = await boosterToken.balanceOf(alice.address);
        expect(balanceAliceBooster).eq(
          parseUnits('10000', boosterTokenDecimals)
        );
      ...
      it('Gives carl 5450 rewardTokens and 10900 boosterTokens and 500 LP', async () => {
        ...
        const balanceCarlBooster = await boosterToken.balanceOf(carl.address);
        expect(balanceCarlBooster).eq(
          parseUnits('14600', boosterTokenDecimals)
        );
      ...
      it('Has an end balance of 250 rewardTokens and 500 boosterTokens which is lost forever', async () => {
        ...
        const balanceFarmReward = await rewardToken.balanceOf(
          boostedFarm.address
        );
        expect(balanceFarmReward).eq(parseUnits('250', rewardTokenDecimals));

        const balanceFarmBooster = await boosterToken.balanceOf(
          boostedFarm.address
        );
