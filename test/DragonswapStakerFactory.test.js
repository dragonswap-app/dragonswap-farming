...
describe('ArborSalesFactory', () => {
    ...
    before(async () => {
        ...
        rewardToken = await tokenFactory.connect(users[0]).deploy('RewardToken', 'RWDT', 18);
        await rewardToken.deployed();

        boosterToken = await tokenFactory.connect(users[0]).deploy('BoosterToken', 'BST', 18);
    ...
    it('Should fail to set classic implementation address if msg.sender is not owner', async () => {
        await expect(
            stakerFactory.connect(owner_two).setImplementationClassic(stakerFarm.address)
        ).to.be.revertedWith('OwnableUnauthorizedAccount').withArgs(owner_two.address);
    });
    ...
    it('Should deploy classic farm correctly', async () => {
        await expect(stakerFactory.deployClassic(rewardToken.address, rewardPerSecond, startTimestamp)).to.emit(stakerFactory,'Deployed');
    });

    it('Should fail to deploy classic farm correctly due to msg.sender not being an owner', async () => {
        await expect(stakerFactory.connect(owner_two).deployClassic(rewardToken.address, rewardPerSecond, startTimestamp)
        ).to.be.revertedWith('OwnableUnauthorizedAccount').withArgs(owner_two.address);
    });
    ...
    it('Should fail to deploy classic farm correctly due to implementation being set as zero address', async () => {
        ...
        await expect(stakerFactory.deployClassic(rewardToken.address, rewardPerSecond, startTimestamp)
    ...
    it('Should confirm that classic farm is deployed through our factory', async () => {
        ...
        await stakerFactory.deployClassic(rewardToken.address, rewardPerSecond, startTimestamp);

        SALE_ADDRESS = await stakerFactory.getLatestDeployment();

        expect(await stakerFactory.isDeployedThroughFactory(SALE_ADDRESS)).to.be.true;
    });

    it('Should confirm that classic farm is not deployed through our factory', async () => {
        newDragonswapStakerFactory = await ethers.getContractFactory('DragonswapStakerFactory');

        newStakerFactory = await newDragonswapStakerFactory.deploy(owner_one.address);
        ...
        await newStakerFactory.deployClassic(rewardToken.address, rewardPerSecond, startTimestamp);
    ...
    it('Should set boosted implementation address correctly', async () => {
        stakerBoostedFarmFactory = await hre.ethers.getContractFactory('DragonswapStakerBoosted');
    ...
    it('Should fail to set boosted implementation address if msg.sender is not owner', async () => {
        await expect(
            stakerFactory.connect(owner_two).setImplementationBoosted(stakerFarm.address)
        ).to.be.revertedWith('OwnableUnauthorizedAccount').withArgs(owner_two.address);
    });
    ...
    it('Should deploy boosted farm correctly', async () => {
        await expect(stakerFactory.deployBoosted(rewardToken.address, boosterToken.address, rewardPerSecond, startTimestamp)).to.emit(stakerFactory,'Deployed');
    });

    it('Should fail to deploy boosted farm correctly due to msg.sender not being an owner', async () => {
        await expect(stakerFactory.connect(owner_two).deployBoosted(rewardToken.address, boosterToken.address, rewardPerSecond, startTimestamp)
        ).to.be.revertedWith('OwnableUnauthorizedAccount').withArgs(owner_two.address);
    });
    ...
    it('Should fail to deploy boosted farm correctly due to implementation being set as zero address', async () => {
        ...
        await expect(stakerFactory.deployBoosted(rewardToken.address, boosterToken.address, rewardPerSecond, startTimestamp)
        ).to.be.revertedWith('ImplementationNotSet');
    ...
    it('Should confirm that boosted farm is deployed through our factory', async () => {
        ...
        await stakerFactory.deployBoosted(rewardToken.address, boosterToken.address, rewardPerSecond, startTimestamp);
    ...
    it('Should confirm that boosted farm is not deployed through our factory', async () => {
        ...
        await newStakerFactory.deployBoosted(rewardToken.address, boosterToken.address, rewardPerSecond, startTimestamp);
    ...
    it('Should get latest deployment address', async () => {
        const newStakerFarm = await stakerFactory.deployClassic(rewardToken.address, rewardPerSecond, startTimestamp);
        const newStakerFarmReceipt = await newStakerFarm.wait();
        const newStakerFarmAddress = newStakerFarmReceipt.logs[0].address;

        expect(await stakerFactory.getLatestDeployment()).to.equal(newStakerFarmAddress);
    });
    ...
    it('Should fail to set new owner correctly due not msg.sender not being owner', async () => {
        await expect(stakerFactory.transferOwnership(owner_one.address)).to.be.revertedWith(
    ...
    it('Should fail to set new owner correctly due to zero address', async () => {
        ...
        ).to.be.revertedWith('OwnableInvalidOwner').withArgs(ZERO_ADDRESS);
    });
});
