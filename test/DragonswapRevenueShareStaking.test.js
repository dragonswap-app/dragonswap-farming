...
describe('Dragonswap Revenue-share Staking', () => {
  ...
  beforeEach(async () => {
    ...
    await this.dragon
      .connect(this.alice)
      .approve(this.dragonStaker.address, ethers.utils.parseEther('100000'));
    await this.dragon
      .connect(this.bob)
      .approve(this.dragonStaker.address, ethers.utils.parseEther('100000'));
    await this.dragon
  ...
  describe('should allow deposits and withdraws', () => {
    it('should allow deposits and withdraws of multiple users', async () => {
      ...
      expect(await this.dragonStaker.totalDeposits()).to.be.equal(
        ethers.utils.parseEther('97')
      );
      // 100 * 0.97 = 97
      expect(await this.dragonStaker.fees()).to.be.equal(
    ...
    it('should allow rewards in Dragon and USDT', async () => {
      ...
      await this.dragonStaker.connect(this.bob).connect(this.bob).withdraw(0);
    ...
    it('should allow adding and removing a rewardToken, only by owner', async () => {
      ...
      expect(await this.dragonStaker.isRewardToken(token1.address)).to.be.equal(
        false
      );
      expect(await this.dragonStaker.rewardTokensCounter()).to.be.equal(1);

      await this.dragonStaker.connect(this.dev).addRewardToken(token1.address);
      await expect(
    ...
    it('should allow setting a new deposit fee, only by owner', async () => {
      ...
      expect(await this.dragon.balanceOf(this.treasury.address)).to.be.equal(
      ...
      await expect(
        this.dragonStaker.connect(this.dev).setDepositFeePercent(2001)
      ).to.be.revertedWith('InvalidValue');

      await this.dragonStaker.connect(this.dev).setDepositFeePercent(2000);
      expect(await this.dragonStaker.depositFeePercent()).to.be.equal(2000);
      ...
      const accumulatedFees = ethers.utils
        .parseEther('3')
        .add(ethers.utils.parseEther('20'));

      const dragonStakerBalance = await this.dragon.balanceOf(
        this.dragonStaker.address
      );

      expect(await this.dragonStaker.fees()).to.be.equal(accumulatedFees);

      await this.dragonStaker.connect(this.dev).withdrawFees();

      expect(await this.dragonStaker.fees()).to.be.equal(0);
    ...
    it('should allow emergency withdraw', async () => {
      ...
      expect(await this.dragonStaker.totalDeposits()).to.be.equal(0);
    ...
    it('should allow owner to sweep stuck tokens that are not rewards', async () => {
      ...
      expect(await stuckToken.balanceOf(this.dragonStaker.address)).to.be.equal(
        0
      );
